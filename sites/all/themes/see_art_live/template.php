...
// */
function see_art_live_css_alter(&$css) {
  $themes=list_themes();
        foreach($css as $key=>$item) {
          if (!in_array($key, $themes['see_art_live']->stylesheets['all'])) {
              unset($css[$key]);
          }
      }
}
...
/*
function see_art_live_js_alter(&$js) {
  if (strpos($_SERVER['REQUEST_URI'], 'user') !== false) {
    $themes=list_themes();
    foreach($js as $key=>$item) {
        if (!in_array($key, $themes['see_art_live']->scripts)) {
            unset($js[$key]);
        }
    }
  }
